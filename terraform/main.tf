...
resource "aws_s3_bucket" "metaflow_experiments_bucket" {
    ...
    Statement = [
      {
        ...
        Effect = "Allow"
        Principal = {
          AWS = module.metaflow.METAFLOW_ECS_S3_ACCESS_IAM_ROLE
        }
        Action = [
          "s3:GetObject",
          "s3:PutObject",
        ...
        Resource = [
          "${aws_s3_bucket.metaflow_experiments_bucket.arn}/*"
        ]
      }
    ]
  })
...
resource "aws_s3_bucket_lifecycle_configuration" "expire_metaflow_objects_after_7_days" {
    bucket = module.metaflow.metaflow_s3_bucket_name


  rule {
    id      = "delete_objects_because_i_am_poor_config"
    status  = "Enabled"
